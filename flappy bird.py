...
screen = pygame.display.set_mode((400,600)) # man hinh la 400*600
pygame.display.set_caption('Flappy Bird.cong')#Ten game
...
tube3_x = 800
tube_width = 50
tube1_height = randint(100,400)   # do cao cua ong dao dong tu 100 den 400
...
score = 0
RED=(255,0,0)
font = pygame.font.SysFont('san',30)#tao font chu
font1 = pygame.font.SysFont('san',50)
background_img = pygame.image.load('images/back.png')#in background
background_img = pygame.transform.scale(background_img,(400,600))
bird_img = pygame.image.load('images/bird.png')#ve chim
bird_img = pygame.transform.scale(bird_img,(35,35))
tube_img = pygame.image.load('images/tube.png')# ve ong
tube_op_img = pygame.image.load('images/tube_op.png')
sand_img = pygame.image.load('images/sand.png')
sand_img=pygame.transform.scale(sand_img,(600,30))
tube1_pass = False
tube2_pass = False
tube3_pass = False
pausing = False
...
while running :
	...
	if tube1_x<-tube_width :    # tao ong moi voi chieu cao random 
		tube1_x = 550
	...
	for tube in tubes :
		if bird.colliderect(tube):
			...
			tube_velocity = 0
			bird_drop_velocity = 0
			game_over_txt=font.render("Game over and score: "+str(score),True,RED)
			screen.blit(game_over_txt,(80,260))
			space_txt =font.render("Space to continue",True,BLUE)
			screen.blit(space_txt,(120,290)) 
			pausing = True   
	for event in pygame.event.get():  # khi ban nhan cac phim tren ban phim  
		if event.type==pygame.QUIT:
			running = False # chuong trinh ko chay nx
